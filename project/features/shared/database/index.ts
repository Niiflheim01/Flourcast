...
const runMigrations = async (database: SQLite.SQLiteDatabase) => {
  try {
    ...
    // Admin/Staff System Migrations
    const hasRole = profileColumns.some(col => col.name === 'role');
    if (!hasRole) {
      console.log('Running migration: Adding role to profiles');
      await database.execAsync("ALTER TABLE profiles ADD COLUMN role TEXT NOT NULL DEFAULT 'admin';");
    }

    const hasAdminPasswordHash = profileColumns.some(col => col.name === 'admin_password_hash');
    if (!hasAdminPasswordHash) {
      console.log('Running migration: Adding admin_password_hash to profiles');
      await database.execAsync('ALTER TABLE profiles ADD COLUMN admin_password_hash TEXT;');
    }

    const hasRecoveryEmail = profileColumns.some(col => col.name === 'recovery_email');
    if (!hasRecoveryEmail) {
      console.log('Running migration: Adding recovery_email to profiles');
      await database.execAsync('ALTER TABLE profiles ADD COLUMN recovery_email TEXT;');
    }

    const hasAdminSetupCompleted = profileColumns.some(col => col.name === 'admin_setup_completed');
    if (!hasAdminSetupCompleted) {
      console.log('Running migration: Adding admin_setup_completed to profiles');
      await database.execAsync('ALTER TABLE profiles ADD COLUMN admin_setup_completed INTEGER NOT NULL DEFAULT 0;');
    }

    const hasCurrentRole = profileColumns.some(col => col.name === 'current_role');
    if (!hasCurrentRole) {
      console.log('Running migration: Adding current_role to profiles');
    ...
    const hasCreatedBy = salesColumns.some(col => col.name === 'created_by_role');
    if (!hasCreatedBy) {
      console.log('Running migration: Adding created_by_role to sales');
      await database.execAsync("ALTER TABLE sales ADD COLUMN created_by_role TEXT DEFAULT 'admin';");
    }
  } catch (error) {
