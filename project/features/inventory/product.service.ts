...
import { Product, ProductWithCategory } from '@/features/shared/types';
...
export class ProductService {
  ...
  static async getIngredients(userId: string): Promise<Product[]> {
    const db = await getDatabase();

    const query = `
      SELECT * FROM products
      WHERE user_id = ? AND product_type = 'ingredient' AND is_active = 1
      ORDER BY name ASC
    `;

    const results = await db.getAllAsync<Product>(query, [userId]);
